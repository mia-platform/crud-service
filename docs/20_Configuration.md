...
|---------------------------------|---------|----------|-------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| MONGODB_URL                     | String  | Required* | -                                         | The MongoDB connection string. It **must** contain the database name on which the collections will be managed. *When `MULTIDB_ENABLED=true`, this variable is **optional**: if omitted, it is automatically derived from `MULTIDB_URL_TEMPLATE` using `DEFAULT_SCOPE`. See [Multi-DB docs](./70_MultiDB.md#mongodb_url-in-multi-db-mode). |
