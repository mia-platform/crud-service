 ...
 * @param {string|undefined} aclColumns a string representing a list of comma separated field names that are allowed to be visualized by current user
 * @param {Array.<string>|undefined} allFieldNames a list of fields names associated to this collection
 * @param {string|undefined} rawProjection a string representing an object that can be employed to manipulate which or how fields should be returned
...
function resolveProjection(
  ...
  if (!clientProjection && !rawProjection) {
    ...
    projection = resolveClientProjectionString(clientProjection, acls)
  }
...
function resolveClientProjectionString(clientProjectionString, _acls) {
  const clientProjection = intoArray(clientProjectionString)
  return removeAclColumns(clientProjection, _acls)
}

function resolveRawProjectionString(rawProjection, _acls, allFieldNames, log) {
  try {
    checkAllowedOperators(rawProjection, rawProjectionDictionary, _acls.length > 0 ? _acls : allFieldNames, log)
...
function removeAclColumnsFromRawProjection(rawProjectionObject, aclColumns) {
  ...
  return filteredFields.reduce(
    (acc, current) => {
      if (rawProjectionObject[current] === 0 || rawProjectionObject[current] !== undefined || rawProjectionObject[current] !== null) {
