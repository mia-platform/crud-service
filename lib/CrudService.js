...
class CrudService {
  ...
  findAll(context, query, projection, sort, skip, limit, _states, isTextSearchQuery) {
    ...
    const sortConfig = getSorting({
      defaultSorting: this._defaultSorting, sort, isTextSearchQuery, mongoProjection,
    })
...
function getSorting({ defaultSorting, sort, isTextSearchQuery, mongoProjection }) {
  let sortConfig = sort
  if (isTextSearchQuery) {
    addTextScoreProjection(mongoProjection)
    if (!sortConfig) {
