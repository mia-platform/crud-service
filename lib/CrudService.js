...
class CrudService {
  ...
  findAll(
    ...
    let cursor = this._mongoCollection
      .find(searchQuery, { ...options, ...cursorOptions })
      .project(projection)
  ...
  aggregate(
    ...
    // TODO: better evaluate the pipeline (stages order)
    const pipeline = [
    ...
    return this._mongoCollection.aggregate(pipeline, aggregationOptions)
  }
  ...
  async patchBulk(
    context, filterUpdateCommands, queryParser, editableFields, aclRows
  ) {
  ...
  async count(context, query, _states, { signal } = { signal: undefined }) {
    const stateQuery = getStateQuery(_states)
    const isQueryValid = query && Object.keys(query).length > 0
    ...
    return this._mongoCollection.countDocuments(searchQuery, { ...options, signal })
  }
