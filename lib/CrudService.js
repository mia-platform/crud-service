...
class CrudService {
  ...
  async upsertMany(context, documents) {
    ...
    const { ok, modifiedCount, nUpserted } = await this._mongoCollection.bulkWrite(operations)

    if (!ok) { throw new Error('upsertMany failed') }

    context.log.debug({ ok, modifiedCount, nUpserted }, 'upsertMany operation executed')
    return nUpserted
  }
