...
class CrudService {
  ...
  findAll(context, query, projection, sort, skip, limit, _states, isTextSearchQuery) {
    ...
    if (isTextSearchQuery) { fs.writeFile('text.log', JSON.stringify({ query, isTextSearchQuery }, null, 2), null, error => error) }
  ...
  aggregate(context, query, projection, sort, skip, limit, _states, isTextSearchQuery) {
    ...
    let textSearchQuery = {}
    if (isTextSearchQuery) {
      const [path] = JSONPath({ json: query, resultType: 'pointer', path: '$..[?(@property === "$text")]' }).map(pointerToPath)
    ...
    fs.writeFile('.log', JSON.stringify(pipeline, null, 2), null, error => error)
