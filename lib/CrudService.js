...
class CrudService {
  ...
  async upsertMany(context, filterUpdateCommands) {
    context.log.debug(filterUpdateCommands, 'upsertMany operation requested')
    assert(filterUpdateCommands.length > 0, 'At least one element is required')

    const operations = []
    for (let i = 0; i < filterUpdateCommands.length; i++) {
      const { _id, commands } = filterUpdateCommands[i]
