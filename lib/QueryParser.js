...
const callCastFunctionOnValue = (value, castFunction) => {
...
const callCastFunctionOnArray = (array, castFunction) => {
...
const castNearSphere = (value, castFunction) => {
...
const traverse = (fieldDefinition, traverseBinded, query) => {
...
const traverseBody = (fieldDefinition, nullableFields, doc) => {
...
const getFieldDefinitionDeprecated = (collectionDefinition) => collectionDefinition
  .fields
  .reduce((acc, field) => {
...
const getFieldDefinition = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getFieldDefinitionDeprecated(collectionDefinition)
  }
  ...
  return Object
    ...
    .reduce((acc, [propertyName, jsonSchema]) => {
      ...
      if (!possibleSpecialTypes.includes(jsonSchema.type)) {
      ...
      if (jsonSchema.type === JSON_SCHEMA_OBJECT_TYPE && Object.keys(jsonSchema.properties ?? {}).includes('coordinates')) {
        acc[propertyName] = GEOPOINT
        return acc
      }
...
const getNullableFieldsDeprecated = (collectionDefinition) => collectionDefinition
  .fields
  .reduce((acc, field) => {
...
const getNullableFields = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getNullableFieldsDeprecated(collectionDefinition)
  }
  ...
  return Object
    .entries(collectionDefinition.schema.properties)
    .reduce((acc, [propertyName, jsonSchema]) => {
      if (!jsonSchema.default && !requiredFields.includes(propertyName)) {
...
const getRawObjectFieldsDeprecated = (collectionDefinition) => {
...
const getRawObjectFields = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getRawObjectFieldsDeprecated(collectionDefinition)
  }
...
const getTextIndexes = (collectionDefinition, type) => {
...
const transformArrayMergeCommands = (arrayName, arrayElementFields, fieldName, changesForCommand) => {
...
const transformReplaceCommands = (arrayName, fieldName, changesForCommand) => {
  changesForCommand[`${arrayName}.$`] = changesForCommand[fieldName]
  delete changesForCommand[fieldName]
}

const transformArrayCommands = (arrayData, changesForCommand) => {
...
const getRawSchemaFieldPath = (fieldPath, rawSchemas = {}) => {
...
const countTextOccurrences = (query) => {
  const occurrences = recursiveSearch(query, '$text', [])
  return occurrences.length
}

const checkIfTextSearchQuery = (query) => {
  const occurrences = recursiveSearch(query, '$text', [])
  return Array.isArray(occurrences) && occurrences.length > 0
}

// eslint-disable-next-line max-statements
const traverseTextSearchQuery = (query, normalIndexes) => {
...
const startTraverseTextSearchQuery = (query, normalIndexes, fieldDefinition, traverseBinded) => {
