...
function buildKeysetFilter(position, sortField, sortDir) {
  ...
  const isSortValueNull = sortValue === null || sortValue === undefined

  // When the sort field is null/missing, the keyset filter must account for
  // MongoDB's BSON ordering where null < any real value.
  if (isSortValueNull) {
    if (sortDir === -1) {
      // DESC with null: we are at the tail (nulls come last in DESC).
      // Only pick up remaining null/missing docs with a later _id.
      return {
        $or: [
          { [sortField]: { $eq: null }, _id: { $gt: _id } },
        ],
      }
    }
    // ASC with null: we are at the head (nulls come first in ASC).
    // Pick up remaining null/missing docs with a later _id, plus ALL non-null docs.
    return {
      $or: [
        { [sortField]: { $eq: null }, _id: { $gt: _id } },
        { [sortField]: { $ne: null } },
  ...
  // DESC with non-null value: null/missing docs come after all real values,
  // so include them in the keyset window.
  if (sortDir === -1) {
    baseOr.push({ [sortField]: { $eq: null } })
  }
