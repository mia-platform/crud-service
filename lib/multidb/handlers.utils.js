...
/**
 * Add `scope` field to handler results that are documents or arrays of documents.
 ...
 * @returns {*} result with scope injected where appropriate
 */
function addScopeToResult(result, scope, reply) {
  if (result === reply || result === null || result === undefined) { return result }
  if (typeof result !== 'object') { return result }

  if (Array.isArray(result)) {
    return result.map((item) => {
      if (item && typeof item === 'object') { return { ...item, scope } }
      return item
    })
  }
