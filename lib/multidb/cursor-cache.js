 ...
 *
 * Stores opaque cursor tokens in a collection on the DEFAULT_SCOPE database
 ...
 *
 * This replaces the in-memory Map that was previously on the crud-adapter,
 * enabling horizontal scaling (multiple crud-service pods share the cache).
 */

const COLLECTION_NAME = '_multidb_cursors'
...
/**
 * Create a MongoDB-backed cursor cache.
 *
 * @param {import('mongodb').Db} db - The DEFAULT_SCOPE database
 * @returns {Promise<object>} Cache interface { get, set, ensureIndexes }
 */
async function createCursorCache(db) {
  const collection = db.collection(COLLECTION_NAME)

  // Ensure TTL index exists (idempotent — createIndex is a no-op if index already exists)
