...
async function multidbPlugin(fastify) {
  ...
  // Step 2: Initialize cursor cache on the DEFAULT_SCOPE database
  const cursorCache = await createCursorCache(fastify.multidb.defaultDb)
  Object.assign(fastify.multidb, { cursorCache })
  fastify.log.info('Multi-DB cursor cache initialized (MongoDB-backed)')

  // Step 3: Register onRoute hook that intercepts collection CRUD routes
  //         and replaces handlers with multi-db versions
  fastify.addHook('onRoute', interceptRoute)

  // Step 4: Extend health check to include multi-db scopes
