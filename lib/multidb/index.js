...
const DATABASE_RESPONSE_PROP = {
  type: 'string',
  description: 'The database (scope) from which this document originates.',
}

/**
 * Inject `database` property into the route's response 200 schema
 ...
 */
function injectDatabaseInResponseSchema(routeOptions) {
  const responseSchema = routeOptions.schema?.response?.['200']
  if (!responseSchema) { return }

  if (responseSchema.type === 'array' && responseSchema.items?.properties) {
    responseSchema.items.properties.database = DATABASE_RESPONSE_PROP
  } else if (responseSchema.properties) {
    responseSchema.properties.database = DATABASE_RESPONSE_PROP
  }
}
...
function injectOptionalScope(routeOptions) {
  ...
  injectDatabaseInResponseSchema(routeOptions)
}
...
/**
 * Create indexes on non-default multidb scopes.
 * loadModels already handles the default scope; this replicates indexes
 * on every additional scope so that all databases stay in sync.
 *
 * @param {import('fastify').FastifyInstance} fastify
 */
async function createIndexesOnNonDefaultScopes(fastify) {
  const nonDefaultScopes = fastify.multidb.scopes
    .filter(scope => scope !== fastify.multidb.defaultScope)

  const tasks = []
  for (const [, model] of Object.entries(fastify.models || {})) {
    const { definition } = model
    if (!definition || model.isView) { continue }

    const { name: collectionName, indexes = [] } = definition
    if (indexes.length === 0) { continue }

    for (const scope of nonDefaultScopes) {
      tasks.push({ scope, collectionName, indexes })
    }
  }

  await Promise.all(tasks.map(async({ scope, collectionName, indexes }) => {
    const scopeCollection = fastify.multidb.dbs[scope].collection(collectionName)
    try {
      await createIndexes(scopeCollection, indexes, PREFIX_OF_INDEX_NAMES_TO_PRESERVE)
      fastify.log.debug({ scope, collectionName }, 'Indexes created on multi-db scope')
    } catch (error) {
      fastify.log.error(
        { cause: error, scope, collectionName },
        'failed to create/update/delete an index on multi-db scope collection'
      )
      throw new Error(
        `failed setting up indexes for collection ${collectionName} on scope ${scope}`,
        { cause: error }
      )
    }
  }))
  ...
  fastify.log.info(
    { scopes: nonDefaultScopes },
    'Multi-DB indexes synchronized on non-default scopes'
  )
}
...
async function multidbPlugin(fastify) {
  ...
  // Step 2b: Create indexes on non-default scopes.
  if (!fastify.config.DISABLE_INDEX_MANAGEMENT) {
    await createIndexesOnNonDefaultScopes(fastify)
  }
