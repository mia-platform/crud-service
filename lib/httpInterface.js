...
function handleGetListLookup(request, reply) {
  ...
  const { acl_rows, acl_read_columns: aclReadColumns } = headers
  ...
  function parseAcl(field) {
    const [key] = Object.keys(field)
    if (aclReadColumns.includes(key) || key === '_id') {
      return true
    }
    return false
  }

  const projection = aclReadColumns
    ? this.lookupProjection.filter(parseAcl)
    : this.lookupProjection
