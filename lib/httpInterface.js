...
module.exports = async function getHttpInterface(fastify, { isView }) {
  ...
  const NESTED_SCHEMAS_BY_ID = {
    [SCHEMAS_ID.GET_LIST]: fastify.jsonSchemaGenerator.generateGetListJSONSchema(),
    [SCHEMAS_ID.GET_ITEM]: fastify.jsonSchemaGenerator.generateGetItemJSONSchema(),
    [SCHEMAS_ID.EXPORT]: fastify.jsonSchemaGenerator.generateExportJSONSchema(),
    [SCHEMAS_ID.POST_ITEM]: fastify.jsonSchemaGenerator.generatePostJSONSchema(),
    [SCHEMAS_ID.POST_BULK]: fastify.jsonSchemaGenerator.generateBulkJSONSchema(),
    [SCHEMAS_ID.DELETE_ITEM]: fastify.jsonSchemaGenerator.generateDeleteJSONSchema(),
    [SCHEMAS_ID.DELETE_LIST]: fastify.jsonSchemaGenerator.generateDeleteListJSONSchema(),
    [SCHEMAS_ID.PATCH_ITEM]: fastify.jsonSchemaGenerator.generatePatchJSONSchema(),
    [SCHEMAS_ID.PATCH_MANY]: fastify.jsonSchemaGenerator.generatePatchManyJSONSchema(),
    [SCHEMAS_ID.PATCH_BULK]: fastify.jsonSchemaGenerator.generatePatchBulkJSONSchema(),
    [SCHEMAS_ID.UPSERT_ONE]: fastify.jsonSchemaGenerator.generateUpsertOneJSONSchema(),
    [SCHEMAS_ID.COUNT]: fastify.jsonSchemaGenerator.generateCountJSONSchema(),
    [SCHEMAS_ID.VALIDATE]: fastify.jsonSchemaGenerator.generateValidateJSONSchema(),
    [SCHEMAS_ID.CHANGE_STATE]: fastify.jsonSchemaGenerator.generateChangeStateJSONSchema(),
    [SCHEMAS_ID.CHANGE_STATE_MANY]: fastify.jsonSchemaGenerator.generateChangeStateManyJSONSchema(),
  }
  ...
  fastify.get('/', {
    schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.GET_LIST],
    config: {
  ...
  fastify.get('/export', {
    schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.EXPORT],
    config: {
      replyType: 'application/x-ndjson',
      serializer: fastNdjsonSerializer(fastJson(NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.GET_ITEM].response['200'])),
    },
  }, handleGetList)
  fastify.get('/:id', { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.GET_ITEM] }, handleGetId)
  fastify.get('/count', { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.COUNT] }, handleCount)

  if (!isView) {
    fastify.post(
      '/',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.POST_ITEM] },
      handleInsertOne
    )
    fastify.post(
      '/validate',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.VALIDATE] },
      handleValidate
    )
    fastify.delete(
      '/:id',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.DELETE_ITEM] },
      handleDeleteId
    )
    fastify.delete(
      '/',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.DELETE_LIST] },
      handleDeleteList
    )
    fastify.patch(
      '/:id',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.PATCH_ITEM] },
      handlePatchId
    )
    fastify.patch(
      '/',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.PATCH_MANY] },
      handlePatchMany
    )
    fastify.post(
      '/upsert-one', { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.UPSERT_ONE] }, handleUpsertOne)
    fastify.post('/bulk', {
      schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.POST_BULK],
    }, handleInsertMany)
    fastify.patch('/bulk', {
      schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.PATCH_BULK],
    }, handlePatchBulk)
    fastify.post(
      '/:id/state',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.CHANGE_STATE] },
      handleChangeStateById
    )
    fastify.post(
      '/state',
      { schema: NESTED_SCHEMAS_BY_ID[SCHEMAS_ID.CHANGE_STATE_MANY] },
...
async function customErrorHandler(error, request, reply) {
  ...
  if (error.message === 'body must NOT have additional properties') {
