...
module.exports = async function getHttpInterface(fastify, options) {
  ...
  if (registerSetters) {
    ...
    const validateImportOptions = getAjvResponseValidationFunction(schemaWithoutFile, { removeAdditional: false })
    fastify.post(
      '/import',
      {
        schema: importSchema,
...
async function handleCollectionImport(request, reply) {
  ...
  const { itemSchema, validateImportOptions } = reply.context.config
