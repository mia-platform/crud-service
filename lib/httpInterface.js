...
function handleGetListLookup(request, reply) {
  ...
  const validate = getAjvResponseValidationFunction(context.schema.response['200'].items)
...
async function handleGetId(request, reply) {
  ...
  const validate = getAjvResponseValidationFunction(context.schema.response['200'])
  validate(response)
  return response
}
