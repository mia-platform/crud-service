...
module.exports = async function getHttpInterface(fastify, options) {
  ...
  const NESTED_SCHEMAS_BY_ID = {
    ...
    [SCHEMAS_ID.POST_FILE]: fastify.jsonSchemaGeneratorWithNested.generateImportJSONSchema(),
...
async function handleCollectionImport(request, reply) {
  ...
  const bodyParser = getFileMimeParser(mimetype)
  if (!bodyParser) {
...
function getFileMimeParser(contentType) {
  const mimeParser = {
