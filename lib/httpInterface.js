...
const { shouldValidateStream, getAjvResponseValidationFunction } = require('./validatorGetters')
...
async function handleGetListLookup(request, reply) {
  ...
  try {
    return await pipeline(
      ...
      shouldValidateStream(request.routeSchema.response['200'].items, this.validateOutput),
      serializer(),
      reply.raw
    )
  } catch (error) {
