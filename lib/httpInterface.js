...
function handleGetListLookup(request, reply) {
  ...
  const validate = getAjvResponseValidationFunction(request.context.schema.response['200'].items)

  this.crudService
    .aggregate(context, mongoQuery, projection, sort, skip, limit, stateArr, isTextSearchQuery)
    .stream()
    .on('error', (error) => {
...
function handleGetList(request, reply) {
  ...
  this.crudService
    .findAll(context, mongoQuery, projection, sort, skip, limit, stateArr, isTextSearchQuery)
    .stream()
    .on('error', (error) => {
...
async function handleGetId(request, reply) {
  ...
  const validate = getAjvResponseValidationFunction(request.context.schema.response['200'])
  validate(response)
  return response
}
