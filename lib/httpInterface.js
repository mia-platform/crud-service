...
module.exports = async function getHttpInterface(fastify, options) {
  ...
  if (registerSetters) {
    ...
    const importSchema = fastify.jsonSchemaGenerator.generateImportJSONSchema()
    fastify.post(
      '/import',
      {
        schema: importSchema,
        config: {
          itemSchema: importSchema.streamBody,
        },
      },
...
async function handleCollectionImport(request, reply) {
  ...
  // Agv cannot validate file types
  ...
  }, request.routeSchema.body)

  const ajv = new Ajv({})
  ajv.addVocabulary(Object.values(SCHEMA_CUSTOM_KEYWORDS))
  const validateBody = ajv.compile(schemaWithoutFile)
  const isValid = validateBody(parsingOptions)
  if (!isValid) {
  ...
  const { crudService, queryParser, castItem } = this
  const { log, crudContext } = request

  let documentIndex = 0
  const { itemSchema } = reply.context.config
  const validate = getAjvResponseValidationFunction(itemSchema)
  const parseDocument = through2.obj((chunk, _enc, callback) => {
    try {
      // To optimize
      castItem(chunk)
      validate(chunk)
      if (validate.errors) { throw validate.errors }
  ...
  try {
    ...
    log.error({ error }, 'Error during import stream')
...
function resolveRawProjectionString(rawProjection, _acls, allFieldNames, log) {
  try {
    ...
    return !Ramda.isEmpty(projection) ? [projection] : []
  } catch (errorMessage) {
