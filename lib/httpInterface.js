...
async function *documentsGenerator(dbCursor, transform, { log, abortController }) {
  try {
    for await (const doc of dbCursor) {
      if (!abortController.signal.aborted) {
        yield transform(doc)
      } else {
        log.warn('aborted reading database cursor')
        break
      }
    }
  } catch (error) {
    log.error({ error }, 'processing error')
    // in case of error, signal that underlying resources should be released
    abortController.abort(error)
  } finally {
    // ensure the cursor is closed no matter the stream outcome was
    await dbCursor.close()
    log.debug('collection cursor closed')
  }
}
...
async function handleGetListLookup(request, reply) {
  ...
  try {
    ...
    const dataStream = ReadableStream.from(documentsGenerator(
      cursor,
      castItem,
      { log: request.log, abortController: ac }
    ))
    const serializers = responseStringifiers({ fields: getExportColumns(projection) })

    reply.raw.on('end', async() => cursor.close())
    reply.raw.on('destroy', async(error) => {
      ac.abort(error)
      await cursor.close()
    })
...
async function handleGetList(request, reply) {
  ...
  try {
    cursor = this.crudService
      .findAll(crudContext, mongoQuery, projection, sort, skip, limit, stateArr, isTextSearchQuery, { signal })
