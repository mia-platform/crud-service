...
const generateArrayProperty = (field, typesMap) => {
  ...
  const itemDefinitions = {
    schema: field.items.schema ?? field.items.properties,
  }
  return {
    type: 'array',
...
const propertiesGetValidation = (collectionDefinition) => {
  ...
  const properties = Object
    ...
    .filter(([propertyName, jsonSchema]) =>
      !typesToIgnoreInQuerystring.has(jsonSchema.type) && propertyName !== MONGOID && propertyName !== __STATE__)
    .reduce((acc, [propertyName, jsonSchema]) => {
      acc[propertyName] = specialTypesValidation[jsonSchema.type]
        ? specialTypesValidation[jsonSchema.type]()
...
const propertiesFilterChangeStateMany = (collectionDefinition) => {
  ...
  const properties = Object
    ...
    .reduce((propertiesAccumulator, [propertyName, jsonSchema]) => {
      ...
      propertiesAccumulator[propertyName] = specialTypesValidation[jsonSchema.type]
        ? ({ ...specialTypesValidation[jsonSchema.type](fieldOptionFilter) })
        : { type: jsonSchema.type }
  ...
  return Object
    ...
    .reduce((properties, [propertyName, jsonSchema]) => {
      const specialTypesType = {
        ...(specialTypes[jsonSchema.type] ? specialTypes[jsonSchema.type]() : undefined),
      }
      properties[propertyName] = specialTypes[jsonSchema.type]
        ? specialTypesType
        : { type: jsonSchema.type, nullable: !jsonSchema.default && !requiredFields.includes(propertyName) }
...
const getArrayProperties = (collectionDefinition, rawPaths = {}) => {
  ...
  const fromFields = Object
    ...
    .reduce((properties, [propertyName, jsonSchema]) => {
      ...
      return {
        ...properties,
        [propertyName]: specialTypesValidation[jsonSchema.items.type]
          ? specialTypesValidation[jsonSchema.items.type](itemDefinitions)
...
const getPropertiesFilteredByType = (collectionDefinition, type) => {
  ...
  return Object
    ...
    .reduce((properties, [propertyName]) => {
      return {
        ...properties,
        [propertyName]: specialTypesSerialization[type] || { type },
      }
    }, {})
}
...
const collectionFieldsArrayOperationsProperties = (collectionDefinition) => {
  ...
  return Object
    ...
    .filter(([, jsonSchema]) => jsonSchema.type === JSON_SCHEMA_ARRAY_TYPE)
    .reduce((properties, [propertyName, jsonSchema]) => {
      const { type: itemType, schema } = jsonSchema.items ?? {}
      ...
      if (itemType === JSON_SCHEMA_OBJECT_TYPE) {
