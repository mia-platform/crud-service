...
module.exports = class JSONSchemaGenerator {
  constructor(
    ...
    this._strictOutputValidation = strictOutputValidation
    ...
    this._baseFileProperty = baseFileProperty(openApiSpecification)
  }
  ...
  generatePatchImportJSONSchema() {
    ...
    const schema = {
      summary: `Update items in the ${this._collectionName} collection by input file. Use /export as template`,
      tags: this._collectionTags,
      consumes: ['multipart/form-data'],
      body: {
        ...schemaDetail.body,
  ...
  /**
   * Returns schema for GET /export endpoint
   * @param {string?} defaultAccept optional string to define the default accept header.
   * defaults to 'application/x-ndjson'
   * @returns {unknown}
   */
  generateExportJSONSchema(defaultAccept = 'application/x-ndjson') {
  ...
  /**
   * Generate the json schema for GET / endpoints
   *
   * @param {boolean} generateRequired optional, defaults to false. if defined, required field is added to the schema
   * (flag not needed for endpoints using $project operator)
   * @returns {unknown}
   */
  generateGetItemJSONSchema(generateRequired = false) {
    ...
    return {
      ...
      response: {
        200: {
          ...
          type: 'object',
          properties: this._serializationProperties,
          additionalProperties: !this._strictOutputValidation,
          ...(generateRequired ? { required: this._requiredFields } : {}),
        },
      },
    }
  }

  generateGetSchemaJSONSchema() {
    const schemaDetail = this.getSchemaDetail(SCHEMAS_ID.GET_SCHEMA)

    return {
      summary: `Returns the schema of ${this._collectionName}`,
      tags: this._collectionTags,
      response: {
        200: {
          ...schemaDetail['response.200'],
...
function propertiesGetValidation(validationProperties, isNewSchema) {
  ...
  Object.keys(properties).forEach(property => {
    if ([
      JSON_SCHEMA_ARRAY_TYPE,
      JSON_SCHEMA_OBJECT_TYPE,
    ].includes(properties[property].type)) {
...
const mongoOperatorSchema = {
  type: 'object',
  // these are the available modifiers of upgrade operators in MongoDB v7
  // (https://www.mongodb.com/docs/manual/reference/operator/update/#modifiers)
  properties: {
    '$': {
      'oneOf':
        [
...
function getArrayPropertiesCompatibility(collectionDefinition, rawPaths = {}) {
  ...
  for (const field of collectionDefinition.fields) {
    ...
    // the following is set only for array of objects
    ...
    fromFields[field.name] = specialTypesValidation[fieldItemType]?.(itemDefinitions) ?? { type: fieldItemType }
    // enable using array modifiers operators, such as $each
    fromFieldsWithMongoSupport[field.name] = {
      oneOf: [
        specialTypesValidation[fieldItemType]?.(itemDefinitions) ?? { type: fieldItemType },
        mongoOperatorSchema,
      ],
    }
  }
...
function getArrayProperties(collectionDefinition, rawPaths = {}) {
  ...
  for (const [propertyName, jsonSchema] of Object.entries(collectionDefinition.schema.properties)) {
    ...
    fromFields[propertyName] = specialTypesValidation[inheritedType]?.(itemDefinitions) ?? { type: inheritedType }
    // enable using array modifiers operators, such as $each
    fromFieldsWithMongoSupport[propertyName] = {
      oneOf: [
        specialTypesValidation[inheritedType]?.(itemDefinitions) ?? { type: inheritedType },
        mongoOperatorSchema,
      ],
    }
  }
...
function getInheritedType(jsonSchema) {
  if (jsonSchema.__mia_configuration?.type) {
    return jsonSchema.__mia_configuration.type
  }
...
function removeFileTypeFromSchema(bodySchema) {
  const schemaWithoutFile = {
    ...bodySchema,
    anyOf: structuredClone(bodySchema).anyOf.map((anyOfschema) => {
...
function baseFileProperty(openApiSpecification) {
  if (openApiSpecification === 'swagger') {
    return {
      type: 'file',
    }
  }
