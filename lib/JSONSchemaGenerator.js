...
const rawObjectTypeSchemaCompatibility = ({ schema } = {}) => {
...
const rawObjectTypeSchema = (itemDefinitions) => {
  if (!itemDefinitions.properties) {
    return rawObjectTypeSchemaCompatibility(itemDefinitions)
  }
  const { required, additionalProperties = true, properties } = itemDefinitions || {}
  return {
    type: 'object',
    additionalProperties,
    ...properties ? { properties } : {},
...
const getInheritedType = (jsonSchema) => {
  const possibleSpecialTypes = [JSON_SCHEMA_OBJECT_TYPE, 'string']

  if (!possibleSpecialTypes.includes(jsonSchema.type)) { return jsonSchema.type }
  if (jsonSchema.type === 'string' && (jsonSchema.format ?? '').startsWith('date')) { return DATE }
  if (jsonSchema.type === JSON_SCHEMA_OBJECT_TYPE && Object.keys(jsonSchema.properties ?? {}).includes('coordinates')) { return GEOPOINT }
...
const generateArrayProperty = (field, typesMap) => {
  if (!field.items.properties) {
    return generateArrayPropertyCompatibility(field, typesMap)
  }
  const itemType = field.items.type
  const itemDefinitions = {
...
const sortRegexDeprecated = (collectionDefinition) => {
...
const sortRegex = (collectionDefinition) => {
  if (!collectionDefinition.schema) { return sortRegexDeprecated(collectionDefinition) }
  const typesToIgnore = new Set([GEOPOINT])
  const orFields = Object
    .entries(collectionDefinition.schema.properties)
    .filter(([, jsonSchema]) => !typesToIgnore.has(jsonSchema.type))
    .map(([name]) => name)
    .join('|')
  const subFieldSuffix = '(\\.([^\\.,])+)*'
  const singleFieldMatcher = `-?(${orFields})${subFieldSuffix}`
...
const propertiesGetValidationDeprecated = (collectionDefinition) => {
  let properties = {}
...
const propertiesGetValidation = (collectionDefinition) => {
  if (!collectionDefinition.schema) { return propertiesGetValidationDeprecated(collectionDefinition) }
  const typesToIgnore = new Set([JSON_SCHEMA_OBJECT_TYPE, JSON_SCHEMA_ARRAY_TYPE])
  const properties = Object
    .entries(collectionDefinition.schema.properties)
    .filter(([propertyName, jsonSchema]) =>
      !typesToIgnore.has(jsonSchema.type) && propertyName !== MONGOID && propertyName !== __STATE__)
    .reduce((acc, [propertyName, jsonSchema]) => {
      ...
      acc[propertyName] = specialTypesValidation[inheritedType]
        ? specialTypesValidation[inheritedType]()
        : { type: inheritedType }
      return acc
    }, {
      [PROJECTION]: {
...
const propertiesFilterChangeStateManyDeprecated = (collectionDefinition) => {
  const typesToIgnore = new Set([GEOPOINT])
  const properties = {}
...
const propertiesFilterChangeStateMany = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return propertiesFilterChangeStateManyDeprecated(collectionDefinition)
  }
  ...
  const properties = Object
    ...
    .reduce((propertiesAccumulator, [propertyName, jsonSchema]) => {
      ...
      propertiesAccumulator[propertyName] = specialTypesValidation[inheritedType]
        ? ({ ...specialTypesValidation[inheritedType](fieldOptionFilter) })
        : { type: inheritedType }

      if (jsonSchema.description) {
        propertiesAccumulator[propertyName].description = jsonSchema.description
      }
...
const collectionFieldsPropertiesDeprecated = (collectionDefinition, validation) => {
...
const collectionFieldsProperties = (collectionDefinition, validation) => {
  if (!collectionDefinition.schema) {
    return collectionFieldsPropertiesDeprecated(collectionDefinition, validation)
  }
  const specialTypes = validation ? specialTypesValidation : specialTypesSerialization
  const { required: requiredFields = [] } = collectionDefinition.schema ?? {}
  ...
  return Object
    ...
    .reduce((properties, [propertyName, jsonSchema]) => {
      ...
      const specialTypesType = {
        ...(specialTypes[inheritedType] ? specialTypes[inheritedType]() : undefined),
      }
      properties[propertyName] = specialTypes[inheritedType]
        ? specialTypesType
        : { type: inheritedType, nullable: !jsonSchema.default && !requiredFields.includes(propertyName) }

      return properties
    }, {})
}

const collectionFieldsRawObjectDeprecated = (collectionDefinition) => {
...
const collectionFieldsRawObject = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return collectionFieldsRawObjectDeprecated(collectionDefinition)
  }
  const arrayOperationsProperties = Object
    .entries(collectionDefinition.schema.properties)
    .filter(([, jsonSchema]) => jsonSchema.type === JSON_SCHEMA_OBJECT_TYPE && !jsonSchema.schema)
...
const getUnsetPatternPropertiesDeprecated = (collectionDefinition) => collectionDefinition
  .fields
  .filter(field => {
...
const getUnsetPatternProperties = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getUnsetPatternPropertiesDeprecated(collectionDefinition)
  }
...
const getArrayPropertiesDeprecated = (collectionDefinition, rawPaths = {}) => {
...
const getArrayProperties = (collectionDefinition, rawPaths = {}) => {
  if (!collectionDefinition.schema) {
    return getArrayPropertiesDeprecated(collectionDefinition, rawPaths)
  }
  ...
  const fromFields = Object
    ...
    .reduce((properties, [propertyName, jsonSchema]) => {
      const itemDefinitions = {
        schema: jsonSchema.items.properties,
      }
      ...
      return {
        ...properties,
        [propertyName]: specialTypesValidation[inheritedType]
...
const getPropertiesFilteredByTypeDeprecated = (collectionDefinition, type) => collectionDefinition
  .fields
  .filter(notMandatory)
...
const getPropertiesFilteredByType = (collectionDefinition, type) => {
  if (!collectionDefinition.schema) {
    return getPropertiesFilteredByTypeDeprecated(collectionDefinition, type)
  }
...
const collectionFieldsArrayOperationsPropertiesDeprecated = (collectionDefinition) => collectionDefinition
  .fields
  .filter(field => field.type === ARRAY)
  .reduce((properties, field) => {
    const itemType = field.items.type
...
const collectionFieldsArrayOperationsProperties = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return collectionFieldsArrayOperationsPropertiesDeprecated(collectionDefinition)
  }
  ...
  return Object
    ...
    .filter(([, jsonSchema]) => jsonSchema.type === JSON_SCHEMA_ARRAY_TYPE)
    .reduce((propertiesAccumulator, [propertyName, jsonSchema]) => {
      const { properties } = jsonSchema.items ?? {}
      ...
      propertiesAccumulator[replacePropertyName] = specialTypesValidation[inheritedType]
        ? ({ ...specialTypesValidation[inheritedType](replaceFieldItemsOption) })
        : { type: inheritedType }
      ...
      if (inheritedType === RAWOBJECTTYPE) {
        ...
        propertiesAccumulator[mergePropertyName] = {
          type: 'object',
          ...jsonSchema.items.schema ? { properties: jsonSchema.items.schema.properties } : {},
          // additionalProperties true to support dot notation
          additionalProperties: true,
        }
      }
...
const unsetPropertiesDeprecated = (collectionDefinition) => collectionDefinition
  .fields
  .filter(notRequired)
  .reduce((properties, field) => {
...
const unsetProperties = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return unsetPropertiesDeprecated(collectionDefinition)
  }
...
const getCurDatePropertiesDeprecated = (collectionDefiniton) => collectionDefiniton
  .fields
  .filter(notMandatory)
...
const getCurDateProperties = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getCurDatePropertiesDeprecated(collectionDefinition)
  }
...
const getRequiredFieldsDeprecated = (fields) => fields
  .filter(field => field.required && !mandatoryFields.has(field.name))
  .map(field => field.name)

const getRequiredFields = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getRequiredFieldsDeprecated(collectionDefinition.fields)
  }
...
const getIdTypeDeprecated = (collectionDefinition) => {
...
const getIdType = (collectionDefinition) => {
  if (!collectionDefinition.schema) {
    return getIdTypeDeprecated(collectionDefinition)
  }
  const response = collectionDefinition.schema.properties[MONGOID]
  const { type: idType } = response
  return idType
}
