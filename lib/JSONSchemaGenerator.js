  ...
  collectionDefinition
    ...
    .forEach((field) => {
      if (field.type !== ARRAY) {
        ...
        const arrayProperties = generateArrayProperty(field, specialTypes)
        collectionFields[field.name] = arrayProperties
        if (field.items.type === RAWOBJECTTYPE) {
          changeStateProperties[field.name] = arrayProperties
        }
      }
