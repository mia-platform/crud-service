...
const generateArrayPropertyDeprecated = (field, typesMap) => {
...
const sortRegex = (collectionDefinition) => {
  ...
  const orFields = Object
    .entries(collectionDefinition.schema)
    ...
    if (!typesToIgnore.has(field.type) && field.name !== MONGOID) {
      if (field.type === ARRAY) {
        if (field.items.type === RAWOBJECTTYPE) {
          properties[field.name] = generateArrayPropertyDeprecated(field, specialTypesSerialization)
        }
  ...
  return collectionDefinition
    ...
    .reduce((properties, field) => {
      if (field.type !== ARRAY) {
        ...
        properties[field.name] = generateArrayPropertyDeprecated(field, specialTypes)
      }
...
const getUnsetPatternProperties = (collectionDefinition) => {
  ...
  return Object
    ...
    .filter(([, jsonSchema]) => {
      ...
      const isArrayOfObject = jsonSchema.type === ARRAY
...
const getArrayProperties = (collectionDefinition, rawPaths = {}) => {
  ...
  const fromFields = Object
    .entries(collectionDefinition.schema)
    .filter(([, jsonSchema]) => jsonSchema.type === ARRAY)
...
const getIdType = (collectionDefinition) => {
  const { type: idType } = !collectionDefinition.schema
    ? (collectionDefinition.fields.find(field => field.name === MONGOID) ?? {})
    : collectionDefinition.schema[MONGOID]
