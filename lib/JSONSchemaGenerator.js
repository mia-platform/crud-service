...
module.exports = class JSONSchemaGenerator {
  constructor(
    ...
    // const serializationProperties = collectionFieldsProperties(structuredClone(collectionDefinition), false)
    const validationProperties = collectionFieldsProperties(structuredClone(collectionDefinition), true)
...
function propertiesGetListValidation(validationProperties, enableLimitConstraints, maxLimit, idType) {
  const sortPattern = sortRegex(validationProperties)
  const properties = {
    ...validationProperties,
...
function propertiesGetExportValidation(validationProperties, idType) {
  const sortPattern = sortRegex(validationProperties)
  return {
    [MONGOID]: {
      ...mongoIdTypeValidator[idType](),
    },
    ...validationProperties,
    ...
    [SORT]: {
      anyOf: [
        ...
        {
          type: 'array',
          items: { type: 'string', pattern: sortPattern },
        },
      ],
      description: 'Sort by the specified property/properties (Start with a "-" to invert the sort order)',
    },
...
function propertiesUpsertCommandsValidation(collectionDefinition, validationProperties) {
  return {
    ...propertiesPatchCommandsValidation(collectionDefinition, validationProperties),
    [SETONINSERTCMD]: {
      type: 'object',
