  ...
  collectionDefinition
    ...
    .forEach((field) => {
      if (field.type !== ARRAY) {
        ...
        collectionFields[field.name] = specialTypes[field.type] ? specialTypesType : { type: field.type }
        if (!typesToIgnoreInSortAndChange.has(field.type)) {
          changeStateProperties[field.name] = specialTypes[field.type]
            ? specialTypesType
            : { type: field.type }
        }
      } else {
        collectionFields[field.name] = generateArrayProperty(field, specialTypes)
        if (field.items.type === RAWOBJECTTYPE) {
          changeStateProperties[field.name] = generateArrayProperty(field, specialTypesSerialization)
        }
      }

      if (field.nullable === true) {
        collectionFields[field.name].nullable = true
      }
      if (field.description) {
        collectionFields[field.name].description = field.description
        if (changeStateProperties[field.name]) {
          changeStateProperties[field.name].description = field.description
        }
      }
      if (field.type === RAWOBJECTTYPE && !field.schema) {
        collectionRawObject[`${field.name}.`] = true
      }

      if (field.required && !mandatoryFields.has(field.name)) {
        required.push(field.name)
      }

      if (!typesToIgnoreInSortAndChange.has(field.type)) {
        orFields.push(field.name)
      }
    })
  ...
  Object
    ...
    .forEach(([propertyName, jsonSchema]) => {
      if (jsonSchema.type === JSON_SCHEMA_ARRAY_TYPE) {
        collectionFields[propertyName] = generateArrayProperty(jsonSchema, specialTypes)
        if (jsonSchema.items.type === JSON_SCHEMA_OBJECT_TYPE) {
          changeStateProperties[propertyName] = generateArrayProperty(jsonSchema, specialTypesSerialization)
        }
      } else {
        const inheritedType = getInheritedType(jsonSchema)
