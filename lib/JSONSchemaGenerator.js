...
module.exports = class JSONSchemaGenerator {
  constructor(
    ...
    const validationProperties = collectionFieldsProperties(collectionDefinition, true)
    const sort = sortRegex(collectionDefinition)
    const validationGetProperties = propertiesGetValidation(structuredClone(validationProperties))
    ...
    this._collectionFieldsRawObject = collectionFieldsRawObject(collectionDefinition)
...
function propertiesGetValidation(validationProperties) {
  const properties = {
    ...validationProperties,
  ...
  //   __STATE__,
  //   ...JSONPath({
  //     path: `$?[@.type === '${JSON_SCHEMA_ARRAY_TYPE}' || @.type === '${JSON_SCHEMA_OBJECT_TYPE}']`,
  //     json: validationProperties,
  //     resultType: 'path',
  //   })
  //     .map(item => item.replace(pathRegex, '')),
  // ]

  Object.keys(properties).forEach(property => {
    if (['array', 'object'].includes(properties[property].type)) { delete properties[property] }
  })
  if (Object.keys(properties).includes('publishDate')) {
    fs.writeFile(`getList-new.log`, JSON.stringify(properties, null, 2), null, error => error)
  }
  return properties
}
...
//         default: 'PUBLIC',
//         description:
//   'Filter by \\_\\_STATE__, multiple states can be specified in OR by providing a comma separated list',
//       },
//       [RAW_PROJECTION]: {
//         type: 'string',
//         description: 'Additional raw stringified projection for MongoDB',
...
//     MONGOID,
//     __STATE__,
//     JSONPath({ path: `$?[@.type === ${JSON_SCHEMA_ARRAY_TYPE} || @.type === ${JSON_SCHEMA_OBJECT_TYPE}]` })
//       .map(item => item.replace(pathRegex, '')),
//   ]

//   objectAndArrays.forEach(property => delete properties[property])
