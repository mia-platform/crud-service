...
const assertSearchableType = ({ encryption = { searchable: false }, type, specialType }) => {
  const isUnsearchableType = unsearchableTypes.includes(specialType ?? type)
...
const retrieveBsonType = (field) => {
  ...
  return bsonAdapterMap[field.specialType ?? field.type]
}
...
const canGenerateProperty = ({ type, encryption, specialType }) =>
  isEncryptionEnabled({ encryption }) || (TYPES_HAS_PROPERTIES.includes(type) && !specialType)
...
function generateForDefinitions(databaseName, definitions, dataKeyMap) {
  const response = definitions.reduce(
    (accumulator, value) => {
