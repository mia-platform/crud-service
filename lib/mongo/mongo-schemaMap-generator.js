...
const assertSearchableType = ({ encryption = { searchable: false }, type }) => {
  const isUnsearchableType = unsearchableTypes.includes(type)
  if (encryption.searchable && isUnsearchableType) {
...
const retrieveBsonType = (field) => {
  ...
  return bsonAdapterMap[field.type]
}
...
const canGenerateProperty = ({ type, encryption }) =>
  isEncryptionEnabled({ encryption }) || TYPES_HAS_PROPERTIES.includes(type)
...
const propertiesLocationMap = {
  [PLAIN_OBJECT]: 'properties',
  [RAWOBJECTTYPE]: 'schema.properties',
}

const retrieveProperties = (object) => {
  const propertyKey = propertiesLocationMap[object.type]
 ...
 */
function generateForDefinition(definition, dataKey) {
  const { fields } = definition
  const properties = retrieveProperties(definition)
...
function generateForDefinitions(databaseName, definitions, dataKeyMap) {
  return definitions.reduce(
    (accumulator, value) => {
      ...
      accumulator[`${databaseName}.${collectionName}`] = generateForDefinition(value, dataKey)
      return accumulator
    },
    {}
  )
