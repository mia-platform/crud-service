...
const compatibilityModelJsonSchema = {
  ...
  properties: {
    ...
    defaultSorting: {
      type: 'object',
      description: 'default sorting applied to CRUD collection',
    ...
    enableLookup: {
      type: 'boolean',
      description: 'employed in the view model definition - if the view has to expose POST, PATCH and DELETE methods and lookups to referenced collections',
    },
    pipeline: {
      type: 'array',
...
const propertySchema = {
  ...
  properties: {
    ...
    __mia_configuration: {
      ...
      // NOTE: Let's keep this to true so whenever these's another metadata to keep in here
      // we can add it without having retrocompatibility issues
      additionalProperties: true,
    },
    nullable: { type: 'boolean' },
    properties: {
...
const modelJsonSchema = {
  ...
  properties: {
    ...
    defaultSorting: {
      type: 'object',
      description: 'mongodb sorting document applied to CRUD collection on find queries, if _s isn\'t defined explicitly.',
      additionalProperties: {
        type: 'number',
        enum: [-1, 1],
      },
