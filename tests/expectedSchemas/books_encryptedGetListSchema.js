...
module.exports = {
  'summary': 'Returns a list of documents in books-encrypted',
  'description': 'Results can be filtered specifying the following parameters:',
  'tags': [
    'Books Encrypted Endpoint',
  ],
  'querystring': {
    'operationId': 'books-encrypted__MIA__getList__MIA__querystring',
    'type': 'object',
    'properties': {
      '_id': {
        'type': 'string',
      ...
      '_s': {
        'anyOf': [
          {
            'type': 'string',
            'pattern': '^-?(_id|name|isbn|price|author|authorAddressId|isPromoted|publishDate|tags|tagIds|additionalInfo|metadata|attachments|editionsDates|updaterId|updatedAt|creatorId|createdAt|__STATE__)(\\.([^\\.,])+)*(,-?(_id|name|isbn|price|author|authorAddressId|isPromoted|publishDate|tags|tagIds|additionalInfo|metadata|attachments|editionsDates|updaterId|updatedAt|creatorId|createdAt|__STATE__)(\\.([^\\.,])+)*)*$',
          },
          {
            'type': 'array',
            'items': {
  ...
  'response': {
    '200': {
      'operationId': 'books-encrypted__MIA__getList__MIA__response.200',
      'type': 'array',
      'items': {
        'type': 'object',
        'properties': {
