...
tap.test('HTTP POST /import', async t => {
  ...
  t.test('should not violate index uniqueness', async t => {
    ...
    const form = new FormData()
    form.append('file', createReadStream(path.join(__dirname, 'filesFixtures/books.json')), { contentType: 'application/json' })
    const response = await fastify.inject({
      method: 'POST',
      url: `${prefix}/import`,
      payload: form,
  ...
  t.test('should return the correct error if a row is invalid', async t => {
    const form = new FormData()
    form.append('file', createReadStream(path.join(__dirname, 'filesFixtures/booksError.json')), { contentType: 'application/json' })
    const response = await fastify.inject({
      method: 'POST',
      url: `${prefix}/import`,
      payload: form,
